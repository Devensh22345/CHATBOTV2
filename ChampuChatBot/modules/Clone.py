...
from ChampuChatBot import ChampuChatBot as app, save_clonebot_owner
...
async def clone_txt(client, message):
    if len(message.command) > 1:
        ...
        try:
            ai = Client(bot_token, API_ID, API_HASH, bot_token=bot_token, plugins=dict(root="ChampuChatBot/mplugin"))
            await ai.start()
            bot = await ai.get_me()
            bot_id = bot.id
        ...
        try:
            ...
            await mi.edit_text(
                f"⚠️ <b>Error:</b>\n\n<code>{e}</code>\n\n**Forward this message to @mai_hu_kira for assistance**"
            )
    else:
